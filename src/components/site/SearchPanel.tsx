...
export function SearchPanel({ items }: { items: Item[] }) {
  ...
  return (
    <div className="search-panel-pro glass-panel glass-border mx-auto max-w-5xl rounded-[1.75rem] p-4 md:p-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="h-14 w-full rounded-full border border-white/14 bg-slate-950/55 pl-12 pr-4 text-sm text-white placeholder:text-slate-400 focus:border-cyan-200"
          />
        </div>
      </div>
      <div className="mt-4 grid gap-2 md:grid-cols-2">
        {results.map((item) => (
          <Link key={`${item.type}-${item.href}`} href={item.href} className="search-result-pro rounded-2xl border border-white/10 bg-white/7 p-4 transition hover:bg-white/12">
